var mainPlatform = {
	...
	bindEvent: function(){
        ...
        $(document).on('click', '.sider-nav .pf-menu-title', function() {
            ...
            if($(this).closest('.no-child').size() > 0) {
	        	...
	        	$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
					...
					content: '<iframe name="iframe"  class="page-iframe" src="'+ $(this).closest('.no-child').data('href') +'" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
					closable: true
				});
            }
        ...
        $(document).on('click', '.sider-nav-s li', function(e){
        	...
        	$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
				title: $(this).text(),
				content: '<iframe name="iframe" class="page-iframe" src="'+ $(this).data('href') +'" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
				closable: true
			});
        });
        //左侧菜单收起
	...
	_createSiderMenu: function(menu, index){
		...
		var menuStr = '<h2 class="pf-model-name">'+
			//'<span class="iconfont">'+ menu[0].icon+'</span>'+
			'<i class="fa '+menu[0].icon+'"></i>&nbsp&nbsp'+
			'<span class="pf-name">'+ menu[0].title +'</span>'+
			'<span class="toggle-icon"></span>'+
			'</h2><ul class="sider-nav">';

		for(var i = 0, len = menu[0].menu.length; i < len; i++){
			var m = menu[0].menu[i],
				mstr = '';
			var str = '';
			...
			if(i==0){
				//默认右侧显示左侧第一个菜单
				childStr += '<li class="active" text="'+ m.title +'" data-href="' + m.href + '"><a href="#">'+ m.title +'</a></li>';
				$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
					title: m.title,
					content: '<iframe name="iframe" class="page-iframe" src="'+ m.href +'" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
					closable: true
				});
			...
			for(var j = 0, jlen = m.children.length; j < jlen; j++){
				...
				if(i==0){
					...
					$('.easyui-tabs1[arrindex='+ index +']').tabs('add',{
						title: child.title,
						content: '<iframe name="iframe" class="page-iframe" src="'+ child.href +'" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
						closable: true
					});
				}else {
	...
	_createTopMenu: function(){
		...
		*/
		$.each(SystemMenu,function(i,val){  //遍历二维数组
			menuStr += '<li class="pf-nav-item project" data-sort="'+ i +'" data-menu="system_menu_" + i>'+
				'<a href="javascript:;">'+
				//'<span class="iconfont">'+ val[0].icon +'</span>'+
				'<i class="topi fa '+val[0].icon+'"></i>&nbsp&nbsp'+
				'<span class="pf-nav-title">'+ val[0].title +'</span>'+
				'</a>'+
				'</li>';
			// 渲染当前
			if (i==0){
				currentIndex = i;
				mainPlatform._createSiderMenu(SystemMenu[i], i);
			}
		})
