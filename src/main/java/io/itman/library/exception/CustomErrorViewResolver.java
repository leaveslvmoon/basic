...
import io.itman.library.util.ApiUtil;
...
public class CustomErrorViewResolver implements ErrorViewResolver {
    ...
    @ResponseBody
    public ApiUtil handle(Exception e, HttpServletRequest request) {
        String getContextPath = request.getPathInfo();
        e.printStackTrace();//向控制台打印异常信息
        StringWriter sw = new StringWriter();
        e.printStackTrace(new PrintWriter(sw, true));
        String strs = sw.toString();
        //System.out.println("异常啦----"+strs);//异常信息存入数据库
        String sql="insert into errorlog (time,errorInfo)\n" +
                "values(NOW(),?)";
        Db.update(sql,strs);
        return new ApiUtil("很抱歉,请稍后再试", "", 0, 0);
    }
