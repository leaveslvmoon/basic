package io.itman.admin.core.quartz;
...
//  @Log(desc = "更新定时任务", type = Log.LOG_TYPE.UPDATE)
  public boolean updateJob(SysJob job) {
    ...
    try {
      ...
      JobKey jobKey = JobKey.jobKey(job.getId().toString(), Scheduler.DEFAULT_GROUP);
  ...
  public boolean remove(SysJob job) {
    ...
    try {
      if (checkJob(job)) {
        ...
        scheduler.deleteJob(JobKey.jobKey(job.getId().toString(), Scheduler.DEFAULT_GROUP));
